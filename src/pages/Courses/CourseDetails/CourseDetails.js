...
const CourseDetails = () => {
  ...
  return (
    <div className="w-full">
      <button
        className="mx-auto flex items-center font-bold my-3"
      ...
      <div
        ref={ref}
        className={`max-w-2xl mx-auto mb-8 p-2 shadow-lg border border-slate-900/10 rounded`}
      >
        <div className="flex items-center justify-between my-3">
          <h2 className="text-xl sm:text-3xl font-bold my-4 text-center">
            {courseName}
          </h2>
          <Pdf targetRef={ref} filename={courseName}>
            {({ toPdf }) => (
              <button
                onClick={toPdf}
                className="flex items-center font-bold gap-x-1"
              >
